...
class InvernVLModule(VLMBaseModule):
    ...
    def post_model_init(self, model, processing_class):
        self.conv_template = (
            model.conv_template if self.conv_template is None else self.conv_template
        )
        self.num_image_token = (
            model.num_image_token
            if self.num_image_token is None
            else self.num_image_token
        )
    ...
    def get_eos_token_id(self, processing_class):
        eos_token_id = processing_class.convert_tokens_to_ids(
            self.conv_template.sep.strip()
        )
        return eos_token_id
    ...
    def prepare_prompt(
        ...
        for example in inputs:
            ...
            processed_list = process_conversation_list(
                conversation_list, system_message
            )
    ...
    def prepare_model_inputs(
        ...
        model_inputs["image_flags"] = torch.ones(
            full_pixel_values.shape[0], dtype=torch.long
        )
